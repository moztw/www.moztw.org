 ...
 *
 * Contributor(s): 
 *     Hung-Teh Lin <piaip@csie.ntu.edu.tw>
...
/* piaip: We don't need nsztool.h anymore */
/* #include "nsztool.h" */
#define MAX_BUF                         4096
...
void ShowUsage(char *name)
{
    ...
    wsprintf(szBuf, "mozip: A Mozilla Installer Zipper v0.2\n"
	    "Contact Hung-Te Lin <piaip@csie.ntu.edu.tw> if you need help.\n\n"
	    "Usage: %s [-u|x|t|d|h] <installer filename> <files [file1] [file2]...>\n", name);
    lstrcat(szBuf, "\n");
    lstrcat(szBuf, "    -u: add/update files into the installer (Default action)\n");
    lstrcat(szBuf, "    -x: extract files inside the installer\n");
    lstrcat(szBuf, "    -t: test (list) files inside the installer\n");
    lstrcat(szBuf, "    -d: delete files inside the installer\n");
    lstrcat(szBuf, "    -h: This help message\n");
    lstrcat(szBuf, "    <installer filename>: name to use for the self-extracting executable\n");
    lstrcat(szBuf, "    <files>: file list to add/extract/delete to the self-extracing executable\n");
...
void AddFile(HANDLE hExe, LPSTR lpszFile)
{
  ...
  printf("Adding File: '%s': ", szResourceName);
  ...
  *(LPDWORD)(lpBufCmp + sizeof(DWORD)) = dwFileSize;
                     ...
                     lpBufCmp, dwFileSizeCmp + (sizeof(DWORD) * 2)))
  {
    ...
    printf("%d/%d(un/compressed) bytes written\n", dwFileSize, dwFileSizeCmp);
  }
...
/* Function to remove a file from a self-extracting .exe file.
 * It sets the resource as length = 0. */
void DelFile(HANDLE hExe, LPSTR lpszFile)
{
  char        szBuf[MAX_BUF];
  char        szResourceName[MAX_BUF];
  LPBYTE      lpBuf;
  LPBYTE      lpBufCmp;
  HRSRC	      hrsrc;
  LPCTSTR     rscType = "FILE";	

  if(!hExe) exit(1);

  ParsePath(lpszFile, szResourceName, sizeof(szResourceName), PP_FILENAME_ONLY);
  strupr(szResourceName);
  printf("Removing File: '%s': ", szResourceName);
  /*
  hrsrc = FindResourceEx((HINSTANCE)hExe,  szResourceName, rscType,
  ...
  if(hrsrc == NULL) {
	...
	printf(lpMsgBuf);
  } else*/ if(!UpdateResource(hExe, "FILE", szResourceName, 
	      MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), NULL, 0))
  {
    PrintError("UpdateResource() failed", ERROR_CODE_SHOW);
    exit(1);
  } else {
...
BOOL CALLBACK ListTypeProc(HANDLE hModule, LPTSTR lpszType, LONG lParam) {
    switch((int)lpszType) {
	case RT_ACCELERATOR:
	    PrintError("RT_ACCELERATOR", ERROR_CODE_HIDE);
	    break;
	case RT_ANICURSOR:
	    PrintError("RT_ANICURSOR", ERROR_CODE_HIDE);
	    break;
	case RT_ANIICON:
	    PrintError("RT_ANIICON", ERROR_CODE_HIDE);
	    break;
	case RT_BITMAP:
	    PrintError("RT_BITMAP", ERROR_CODE_HIDE);
	    break;
	case RT_CURSOR:
	    PrintError("RT_CURSOR", ERROR_CODE_HIDE);
	    break;
	case RT_DIALOG:
	    PrintError("RT_DIALOG", ERROR_CODE_HIDE);
	    break;
	case RT_FONT:
	    PrintError("RT_FONT", ERROR_CODE_HIDE);
	    break;
	case RT_FONTDIR:
	    PrintError("RT_FONTDIR", ERROR_CODE_HIDE);
	    break;
	case RT_GROUP_CURSOR:
	    PrintError("RT_GROUP_CURSOR", ERROR_CODE_HIDE);
	    break;
	case RT_GROUP_ICON:
	    PrintError("RT_GROUP_ICON", ERROR_CODE_HIDE);
	    break;
	case RT_ICON:
	    PrintError("RT_ICON", ERROR_CODE_HIDE);
	    break;
	case RT_MENU:
	    PrintError("RT_MENU", ERROR_CODE_HIDE);
	    break;
	case RT_MESSAGETABLE:
	    PrintError("RT_MESSAGETABLE", ERROR_CODE_HIDE);
	    break;
	case RT_RCDATA:
	    PrintError("RT_RCDATA", ERROR_CODE_HIDE);
	    break;
	case RT_STRING:
	    PrintError("RT_STRING", ERROR_CODE_HIDE);
	    break;
	case RT_VERSION:
	    PrintError("RT_VERSION", ERROR_CODE_HIDE);
	    break;
	default:
	    PrintError(lpszType, ERROR_CODE_HIDE);
    }
      return TRUE;
}

BOOL APIENTRY TestFilesProc(HANDLE hModule, LPCTSTR lpszType, LPTSTR lpszName, LONG lParam)
{
  char    szBuf[MAX_BUF];
  HRSRC   hResInfo;
  ...
  dwSizeUnCmp  = *(LPDWORD)(lpBytes + sizeof(DWORD));
    ...
    printf("compressed/uncompressed: %d/%d bytes\n", dwSize, dwSizeUnCmp);
...
BOOL APIENTRY ExtractFilesProc(HANDLE hModule, LPCTSTR lpszType, LPTSTR lpszName, LONG lParam)
{
  ...
  // Create the file
  hFile = CreateFile(lpszName, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow)
{
  ...
  GetArgV(lpszCmdLine, iArg++, szSeaExe, sizeof(szSeaExe));
  if(lstrcmpi(szSeaExe, "-h") == 0 || lstrcmpi(szSeaExe, "-?") == 0) {
    ShowUsage(szBuf);
    exit(1);
  } else
  if(lstrcmpi(szSeaExe, "-x") == 0 || lstrcmpi(szSeaExe, "-t") == 0
    || lstrcmpi(szSeaExe, "-m") == 0 || lstrcmpi(szSeaExe, "-l") == 0) {
      action = szSeaExe[1];

    GetArgV(lpszCmdLine, iArg++, szSeaExe, sizeof(szSeaExe));
    if(action == 'm')
	printf("Extracting translation table from: %s\n", szSeaExe);
    else
	printf("%sing Self-installer: %s\n", (action == 'x'?"Extract":"Test"),szSeaExe);
    hExe = LoadLibrary(szSeaExe);
    if(hExe == 0) {
	printf("Cannot load %s!!\n", szSeaExe);
	return 2;
    }
    if (action == 'm') {
	EnumResourceTypes(hExe, (ENUMRESTYPEPROC) ListTypeProc, 0);
    } else
    if (EnumResourceNames(hExe, "FILE", (ENUMRESNAMEPROC)
	    (action == 'x'?ExtractFilesProc:TestFilesProc), (LONG)szSeaExe) ==0) {
	LPVOID lpMsgBuf;
    ...
    return 0;
  } else if(lstrcmpi(szSeaExe, "-g") == 0)
  {
    /* The first parameter is "-g".
     * Create a new nszip that contains nsinstall.exe in itself */

    GetArgV(lpszCmdLine, iArg++, szNsZipName, sizeof(szNsZipName));
    GetArgV(lpszCmdLine, iArg++, szSeaExe, sizeof(szSeaExe));
  ...
  {
    if(lstrcmpi(szSeaExe, "-u") == 0 || lstrcmpi(szSeaExe, "-d") == 0) {
	action = szSeaExe[1];
	GetArgV(lpszCmdLine, iArg++, szSeaExe, sizeof(szSeaExe));
    }
  ...
  for(i = iArg; i < iArgC; i++)
  {
	...
	if(action == 'u')
	    ...
	    printf("Unknown error: command = %c\n", action);
	}
      }
