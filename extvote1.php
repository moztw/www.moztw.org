...
{
    if (file_exists("vote/voteuser1"))
    {
        $data=file("vote/voteuser1");
...
if ($userid!='')
{
	...
	//處理使用者投票
	    if ($_POST['submit']=="Vote" && $voted!="Y")
	    {
	        ...
	        $data=file("vote/vote1.dat");        //讀入投票資訊
	...
	//觀看投票狀況
		...
		//echo $data[0]."<br>".$data[1]."<br>".$data[2]."<br>".$data[3]."<br>";
	        ...
	        //輸出
	        $votehtml= "<table width=\"100%\" border=\"0\" class=\"sortable\" id=\"sort\"><tr><th>佈景名稱</th><th>得票數</th><th></th><th style='width: 60%;'></th></tr>";
	...
	//主畫面
	    ...
	    if($psok != "Y")
	    {
            ...
            else
            {
	            ...
	            if (file_exists("vote/vote1.dat"))
	            {
	                $data=file("vote/vote1.dat");
	                echo "<font color=blue><b>".trim($data[1])."</b></font><br><br>";
	                    ...
	                    請投下神聖的選票，您最多可以勾選<font color=red><? echo trim($data[2]); ?></font>個佈景：<br><br>
	                    <font color="#8000FF">
	                    <?
